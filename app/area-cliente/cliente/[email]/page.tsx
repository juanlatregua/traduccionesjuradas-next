  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-12">
      ...
      <section className="mt-3 rounded-3xl border border-cream bg-card p-6 shadow-sm sm:p-8">
        <p className="text-xs font-semibold uppercase tracking-wide text-bleu">Cliente que gestionas</p>
        <h1 className="mt-2 text-2xl font-bold tracking-tight text-encre sm:text-3xl">
          {target.companyName || target.name || target.email}
        </h1>
        <p className="mt-1 text-sm text-sepia">{target.email} · solo consulta</p>
      </section>

      {/* Pedidos + descargas */}
      <section className="mt-6 rounded-3xl border border-cream bg-card p-6 shadow-sm sm:p-8">
        <h2 className="text-lg font-semibold text-encre">Pedidos</h2>
        {orders.length === 0 ? (
          <p className="mt-3 text-sm text-sepia">Sin pedidos.</p>
        ) : (
      ...
      {quotes.length > 0 && (
        <section className="mt-6 rounded-3xl border border-cream bg-card p-6 shadow-sm sm:p-8">
          <h2 className="text-lg font-semibold text-encre">Presupuestos</h2>
          <div className="mt-4 overflow-x-auto rounded-2xl border border-cream">
            <table className="w-full text-left text-sm">
              <thead className="bg-parchment text-xs uppercase tracking-wide text-graphite">
                <tr>
              ...
              <tbody>
                {quotes.map((q) => (
                  <tr key={q.id} className="border-t border-cream">
                    ...
                    <td className="px-4 py-3 text-sepia">{q.paidAt ? "Pagado" : q.status}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {invoices.length > 0 && (
        <section className="mt-6 rounded-3xl border border-cream bg-card p-6 shadow-sm sm:p-8">
          <h2 className="text-lg font-semibold text-encre">Facturas</h2>
