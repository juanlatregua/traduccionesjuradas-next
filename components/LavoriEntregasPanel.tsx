...
// fichero) y un clic la envía por el carril de entrega existente (/delivery:
...
export default function LavoriEntregasPanel({
  ...
  async function enviarAlCliente() {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        throw new Error(data?.error || "No se pudo enviar la entrega al cliente.");
      }
      setDone(true);
      router.refresh();
    } catch (err: any) {
    ...
    <div className="mb-4 rounded-xl border border-violet-500/30 bg-violet-500/10 p-3">
        ...
        {entregas.map((e, i) => (
          <li key={i} className="text-sm text-slate-300">
            ...
            <span className="text-xs text-slate-400">
              ...
              {e.enviada ? " · ya enviada al cliente" : " · SIN enviar"}
            </span>
          </li>
        ))}
      </ul>
      {pendientes.length > 0 && !done ? (
        <div className="mt-3 border-t border-violet-500/20 pt-3">
          ...
          >
            {sending ? "Enviando…" : `Revisar y enviar al cliente (${pendientes.length})`}
          </button>
          {error && <p className="mt-2 text-xs text-rose-400">{error}</p>}
        </div>
      ) : (
        <p className="mt-3 border-t border-violet-500/20 pt-3 text-xs text-emerald-300">
          {done ? "Entrega enviada al cliente ✓" : "Todas las entregas de lavori ya fueron enviadas al cliente."}
        </p>
      )}
    </div>
  );
